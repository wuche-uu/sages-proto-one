...
namespace HutongGames.PlayMaker.Actions
{
	...
	[Tooltip("Move a GameObject along a set of Waypoints defining a bezier curve")]
	public class LeanTweenMoveAlongWaypoints : FsmStateAction
	{
		public enum LTLoop
		{
			once,
		...
		[RequiredField]
		[Tooltip("A set of waypoints that define the curve(s), must be sets of 4")]
		...
		public override void OnEnter()
		{
			...
			for (var i=0; i<len; i++) {
				tweenVector [i] = Waypoints[i].Value.transform.position;
			}
			...
			LTDescr tween  = LeanTween.moveLocal(go, tweenVector, time.Value);
