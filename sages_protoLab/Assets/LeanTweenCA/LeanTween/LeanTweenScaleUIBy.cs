...
namespace HutongGames.PlayMaker.Actions
{
	...
	[Tooltip("Add Scale to a Unity UI ")]
	public class LeanTweenScaleByUI : FsmStateAction
	{
		public enum LTLoop
		{
			once,
		...
		public override void OnEnter()
		{
			...
			Vector3 final = go.transform.localScale + vector.Value;
			
			Fsm.Event (onStartEvent);
			
			LTDescr tween = LeanTween.scale(go.GetComponent<RectTransform>(), final, time.Value);
